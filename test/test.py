...
def main() :
    ...
    parser.add_argument('--pid', help='PID implementation to use.', choices=['FastPID', 'SmallPID'], default='SmallPID')
    ...
    if args.test == 'reference' :
        ...
        # factory_f = process.DifferentialFactory(lambda x : math.log((x+0.1) * 0.1) * 0.1 )
        # ref = process.Process(reference, 100, args.n, factory_function = factory_f)
        # dut = process.Process(pid, 100, args.n, factory_function = factory_f)
        ref = process.Process(reference, 100, args.n)
        dut = process.Process(pid, 100, args.n)
        ref.run()
        dut.run()

        fig, ax = plt.subplots(num=None, figsize=(25,15))
        setline = plt.plot(ref.setpoint, '', label='Setpoint')
        refline = plt.plot(ref.output, '--', label='Reference')
        outline = plt.plot(dut.output, '', label='Output/Feedback')
        plt.legend(['Setpoint', 'Reference', 'Out/Feedback'])
